...
class RfqData:
    ...
    def from_dict(d: dict) -> "RfqData":
        ...
        suppliers = [
            Supplier(ten_ncc=s.get("ten_ncc", ""),
...
def fill_pmt_sheet(ws, data: RfqData):
    ...
    for idx, item in enumerate(data.items):
        ...
        ws.cell(row=row, column=2).value = item.ten_hang
        ...
        for ncc_idx, block in enumerate(schema.ncc_blocks):
            ...
            ws.cell(row=row, column=mo_ta_col).value = quote.mo_ta if quote else ""
            ws.cell(row=row, column=sl_col).value = item.so_luong
            ws.cell(row=row, column=hang_col).value = quote.hang if quote else ""
...
def _write_pmt_summary(ws, schema: ItemTableSchema, offset: int, data: RfqData):
    ...
    rows_spec = [
        ...
        (rows["warranty"], "Thời gian bảo hành/ Warranty period", data.thoi_gian_bao_hanh),
...
def _write_cbe_summary(ws, schema: ItemTableSchema, grow, data: RfqData):
    ...
    def clear_block(row, block):
        ws.merge_cells(f"{get_column_letter(block.cols[0])}{row}:{get_column_letter(block.cols[-1])}{row}")
